...
public class Node4 {
  ...
  public final Map<Long, CompletableFuture<JsonObject>> rpcs
    = new HashMap<Long, CompletableFuture<JsonObject>>();
  ...
  public Node4 on(String type, Consumer<Message> handler) {
  ...
  public void log(String message) {
    ...
    df.setTimeZone(tz);
    System.err.println(STR."\{df.format(new Date())} \{message}");
    System.err.flush();
  }
  ...
  public void send(final Message message) {
    log(STR."Sending  \{message.toJson()}");
  ...
  public void handleMessage(Message message) {
    ...
    try {
      ...
      String text = STR."Unexpected exception handling \{message}: \{e}\n\{sw}";
      log(text);
  ...
  public void main() {
    ...
    try {
      while (true) {
        ...
        Thread.ofVirtual().start(() -> handleMessage(message));
      }
    } catch (Throwable e) {
      log(STR."Fatal error! \{e}");
